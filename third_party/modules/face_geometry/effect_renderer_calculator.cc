...
class EffectRendererCalculator : public CalculatorBase {
 public:
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    return gpu_helper_.RunInGlContext([this, cc]() -> absl::Status {
      ...
      const auto& multi_face_geometry =  cc->Inputs().Tag(kMultiFaceGeometryTag).IsEmpty() ? empty_multi_face_geometry : cc->Inputs()
        .Tag(kMultiFaceGeometryTag).Get<std::vector<face_geometry::FaceGeometry>>();
  ...
  ~EffectRendererCalculator() { gpu_helper_.RunInGlContext([this]() { effect_renderer_.reset(); });
  }
 ...
 private:
  static absl::StatusOr<ImageFrame> ReadTextureFromFile( const std::string& texture_path) {
    ...
    std::vector<char> texture_blob_vector(texture_blob.begin(), texture_blob.end()); //file
