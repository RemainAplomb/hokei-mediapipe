# Copyright 2020-2022 Kanstantsin Sokal.
#
...
def Main():
    file_in = open("occluder.obj", "r")
    file_out = open("occluder.pbtxt", "w")
    ...
    for line in file_in.readlines():
        ...
        if tokens[0] == "v":
            ...
            v_position_ids = [int(token.split('/')[0]) -
                              1 for token in tokens[1:]]
            v_tex_coord_ids = [int(token.split('/')[1]) -
                               1 for token in tokens[1:]]
            v_compressed_ids = []
