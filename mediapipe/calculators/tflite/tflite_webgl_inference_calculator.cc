...
#include "mediapipe/calculators/tflite/tflite_webgl_inference_calculator.pb.h"
...
constexpr char kTensorsTag[] = "TENSOR";
constexpr char kTensorsGpuTag[] = "TENSOR_GPU";
}  // namespace
...
// TfLiteWebGlInferenceCalculator File Layout:
//  * Header
//  * Core
//  * Aux
namespace mediapipe {
...
//
class TfLiteWebGlInferenceCalculator : public CalculatorBase {
 public:
...
REGISTER_CALCULATOR(TfLiteWebGlInferenceCalculator);
...
absl::Status TfLiteWebGlInferenceCalculator::GetContract(CalculatorContract* cc) {
 // RET_CHECK(cc->Inputs().HasTag("IMAGE_GPU"));
  
  //RET_CHECK(cc->Outputs().HasTag(kTensorsTag) );
  cc->Inputs().Tag("IMAGE_GPU").Set<std::vector<GpuTensor>>();
...
absl::Status TfLiteWebGlInferenceCalculator::Open(CalculatorContext* cc) {
...
absl::Status TfLiteWebGlInferenceCalculator::Process(CalculatorContext* cc) {
  return RunInContextIfNeeded([this, cc]() -> absl::Status {
    ...
    if (gpu_inference_ && gpu_input_ && gpu_output_) {
      ...
      command_buffer.label = @"TfLiteWebGlInferenceCalculator";
      RET_CHECK(
...
absl::Status TfLiteWebGlInferenceCalculator::WriteKernelsToFile() {
#if MEDIAPIPE_TFLITE_GL_INFERENCE && defined(MEDIAPIPE_ANDROID)
  if (use_kernel_caching_) {
    // Save kernel file.
...
absl::Status TfLiteWebGlInferenceCalculator::Close(CalculatorContext* cc) {
...
absl::Status TfLiteWebGlInferenceCalculator::ProcessInputsCpu(
...
absl::Status TfLiteWebGlInferenceCalculator::ProcessInputsGpu(
...
absl::Status TfLiteWebGlInferenceCalculator::ProcessOutputsCpu(
    CalculatorContext* cc,
    std::unique_ptr<std::vector<TfLiteTensor>> output_tensors_cpu) {
...
absl::Status TfLiteWebGlInferenceCalculator::ProcessOutputsGpu(
    CalculatorContext* cc,
    std::unique_ptr<std::vector<TfLiteTensor>> output_tensors_cpu,
    std::unique_ptr<std::vector<GpuTensor>> output_tensors_gpu) {
  if (use_advanced_gpu_api_) {
...
absl::Status TfLiteWebGlInferenceCalculator::ReadKernelsFromFile() {
#if MEDIAPIPE_TFLITE_GL_INFERENCE && defined(MEDIAPIPE_ANDROID)
  if (use_kernel_caching_) {
...
absl::Status TfLiteWebGlInferenceCalculator::InitTFLiteGPURunner(
    CalculatorContext* cc) {
#if MEDIAPIPE_TFLITE_GL_INFERENCE
...
absl::Status TfLiteWebGlInferenceCalculator::LoadModel(CalculatorContext* cc) {
  if (use_advanced_gpu_api_) {
...
absl::StatusOr<Packet> TfLiteWebGlInferenceCalculator::GetModelAsPacket(
...
absl::Status TfLiteWebGlInferenceCalculator::LoadDelegate(CalculatorContext* cc) {
