...
class TfLiteInferenceCalculator : public CalculatorBase {
 ...
 private:
  ...
  absl::Status ProcessInputsCpu(CalculatorContext* cc, std::vector<TfLiteTensor>* output_tensors_cpu);
  absl::Status ProcessOutputsCpu( CalculatorContext* cc, std::unique_ptr<std::vector<TfLiteTensor>> output_tensors_cpu);
  absl::Status ProcessInputsGpu(CalculatorContext* cc, std::vector<GpuTensor>* output_tensors_gpu);
  absl::Status ProcessOutputsGpu( CalculatorContext* cc, std::unique_ptr<std::vector<TfLiteTensor>> output_tensors_cpu, std::unique_ptr<std::vector<GpuTensor>> output_tensors_gpu);
...
#if MEDIAPIPE_TFLITE_GPU_SUPPORTED
  const auto& options = cc->template Options<::mediapipe::TfLiteInferenceCalculatorOptions>();
...
absl::Status TfLiteInferenceCalculator::GetContract(CalculatorContract* cc) {
  RET_CHECK(cc->Inputs().HasTag(kTensorsTag) ^ cc->Inputs().HasTag(kTensorsGpuTag));
  RET_CHECK(cc->Outputs().HasTag(kTensorsTag) ^ cc->Outputs().HasTag(kTensorsGpuTag));

  const auto& options = cc->Options<::mediapipe::TfLiteInferenceCalculatorOptions>();
  RET_CHECK(!options.model_path().empty() ^ cc->InputSidePackets().HasTag(kModelTag)) << "Either model as side packet or model path in options is required.";
