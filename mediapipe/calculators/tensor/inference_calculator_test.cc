...
void DoSmokeTest(const std::string& graph_proto) {
  ...
  input_vec->emplace_back(Tensor::ElementType::kFloat32, Tensor::Shape{1, height, width, channels});
  {
  ...
  CalculatorGraphConfig graph_config = ParseTextProtoOrDie<CalculatorGraphConfig>(graph_proto);
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream( "tensor_in", Adopt(input_vec.release()).At(Timestamp(0))));
