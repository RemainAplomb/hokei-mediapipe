...
TEST_P(InferenceCalculatorTest, TestFaceDetection) {
  CalculatorGraphConfig config;
  ASSERT_TRUE(LoadTestGraph(&config, file::JoinPath(GetTestRootDir(), "mediapipe/calculators/tensor/testdata/face_detection_test.binarypb")));
  ...
  // Load test image.
  std::unique_ptr<ImageFrame> input_image = LoadTestPng(file::JoinPath(GetTestRootDir(), "mediapipe/objc/testdata/sergey.png"));
  ASSERT_THAT(input_image, testing::NotNull());

  std::unique_ptr<ImageFrame> expected_image = LoadTestPng(file::JoinPath(GetTestRootDir(), "mediapipe/calculators/tensor/testdata/face_detection_expected.png"));
  ASSERT_THAT(expected_image, testing::NotNull());

  std::string binary;
  Detection expected_detection;
  MP_ASSERT_OK( file::GetContents(file::JoinPath(GetTestRootDir(), "mediapipe/calculators/tensor/testdata/expected_detection.binarypb"), &binary));
