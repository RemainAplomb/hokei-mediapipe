...
namespace mediapipe
{
  ...
  private:
    absl::Status CreateRenderTargetCpu(CalculatorContext *cc, std::unique_ptr<cv::Mat> &image_mat, ImageFormat::Format *target_format);
  ...
  absl::Status AnnotationOverlayCalculator::GetContract(CalculatorContract *cc)
  {
    ...
    if (cc->Inputs().HasTag(kImageFrameTag) && cc->Inputs().HasTag(kGpuBufferTag))
    {
      return absl::InternalError("Cannot have multiple input images.");
    }
    if (cc->Inputs().HasTag(kGpuBufferTag) != cc->Outputs().HasTag(kGpuBufferTag))
    {
  ...
  absl::Status AnnotationOverlayCalculator::Process(CalculatorContext *cc)
  {
    ...
    if (use_gpu_)
    {
      ...
      if (cc->Inputs().HasTag(kGpuBufferTag))
      {
        MP_RETURN_IF_ERROR((CreateRenderTargetGpu<mediapipe::GpuBuffer, kGpuBufferTag>(cc, image_mat)));
      }
#endif // !MEDIAPIPE_DISABLE_GPU
    }
    else
    ...
    if (use_gpu_)
    {
      ...
      MP_RETURN_IF_ERROR(
          gpu_helper_.RunInGlContext([this, cc, image_mat_ptr]() -> absl::Status
                                     { return RenderToGpu<mediapipe::GpuBuffer, kGpuBufferTag>(cc, image_mat_ptr); }));
#endif // !MEDIAPIPE_DISABLE_GPU
    }
    else
    {
  ...
  absl::Status AnnotationOverlayCalculator::RenderToCpu(CalculatorContext *cc, const ImageFormat::Format &target_format, uchar *data_image)
  {
    auto output_frame = absl::make_unique<ImageFrame>(target_format, renderer_->GetImageWidth(), renderer_->GetImageHeight());

#if !MEDIAPIPE_DISABLE_GPU
    output_frame->CopyPixelData(target_format, renderer_->GetImageWidth(), renderer_->GetImageHeight(), data_image, ImageFrame::kGlDefaultAlignmentBoundary);
#else
    output_frame->CopyPixelData(target_format, renderer_->GetImageWidth(), renderer_->GetImageHeight(), data_image, ImageFrame::kDefaultAlignmentBoundary);
#endif // !MEDIAPIPE_DISABLE_GPU
  ...
  template <typename Type, const char *Tag>
  absl::Status AnnotationOverlayCalculator::RenderToGpu(CalculatorContext *cc, uchar *overlay_image)
  {
#if !MEDIAPIPE_DISABLE_GPU
    ...
    // Upload render target to GPU.
    {
      ...
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width_canvas_, height_canvas_, GL_RGB, GL_UNSIGNED_BYTE, overlay_image);
  ...
  absl::Status AnnotationOverlayCalculator::CreateRenderTargetCpu(CalculatorContext *cc, std::unique_ptr<cv::Mat> &image_mat, ImageFormat::Format *target_format)
  {
    if (image_frame_available_)
    {
      ...
      image_mat = absl::make_unique<cv::Mat>(input_frame.Height(), input_frame.Width(), target_mat_type);
  ...
  template <typename Type, const char *Tag>
  absl::Status AnnotationOverlayCalculator::CreateRenderTargetGpu(CalculatorContext *cc, std::unique_ptr<cv::Mat> &image_mat)
  {
#if !MEDIAPIPE_DISABLE_GPU
    if (image_frame_available_)
    {
      ...
      image_mat = absl::make_unique<cv::Mat>(height_canvas_, width_canvas_, CV_8UC3);
      memset(image_mat->data, kAnnotationBackgroundColor, height_canvas_ * width_canvas_ * image_mat->elemSize());
    }
    else
    {
      image_mat = absl::make_unique<cv::Mat>(height_canvas_, width_canvas_, CV_8UC3, cv::Scalar(options_.canvas_color().r(), options_.canvas_color().g(), options_.canvas_color().b()));
    }
#endif // !MEDIAPIPE_DISABLE_GPU
  ...
  DEFAULT_PRECISION(mediump, float)
    ...
    mediapipe::GlhCreateProgram(mediapipe::kBasicVertexShader, frag_src.c_str(), NUM_ATTRIBUTES, (const GLchar **)&attr_name[0], attr_location, &program_);
    ...
    // Init texture for opencv rendered frame.
    {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, width_canvas_, height_canvas_, 0, GL_RGB, GL_UNSIGNED_BYTE, nullptr);
