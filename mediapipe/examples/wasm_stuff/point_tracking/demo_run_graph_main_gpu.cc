...
absl::Status RunMPPGraph() {
  ...
  std::cout << "Initialising the calculator graph" << std::endl;
  mediapipe::CalculatorGraph graph;
  MP_RETURN_IF_ERROR(graph.Initialize(config));

  std::cout << "Initialise the gpu." << std::endl;
  ...
  while (grab_frames) {
    ...
    if (save_video) {
      if (!writer.isOpened()) {
        std::cout << "Preparing video writer.." << std::endl;
...
int main(int argc, char** argv) {
  ...
  if (!run_status.ok()) {
    std::cout << "Could Not run the graph: " << run_status.message()<< std::endl;;
    return EXIT_FAILURE;
  } else {
