...
class GlScalerCalculator : public CalculatorBase {
 public:
  ...
  void GetOutputPadding(int src_width, int src_height, int dst_width, int dst_height, float* top_bottom_padding, float* left_right_padding);
...
absl::Status GlScalerCalculator::Open(CalculatorContext* cc) {
  ...
  int rotation_ccw = 0;
  const auto& options = tool::RetrieveOptions(cc->Options<GlScalerCalculatorOptions>(), cc->InputSidePackets(), "OPTIONS");
  ...
  if (options.has_scale_mode()) {
    scale_mode_ = FrameScaleModeFromProto(options.scale_mode(), FrameScaleMode::kStretch);
  }

  if (HasTagOrIndex(cc->InputSidePackets(), "OUTPUT_DIMENSIONS", 1)) {
    const auto& dimensions = TagOrIndex(cc->InputSidePackets(), "OUTPUT_DIMENSIONS", 1).Get<DimensionsPacketType>();
...
absl::Status GlScalerCalculator::Process(CalculatorContext* cc) {
  if (cc->Inputs().HasTag(kOutputDimensionsTag)) {
    ...
    const auto& dimensions = cc->Inputs().Tag(kOutputDimensionsTag).Get<DimensionsPacketType>();
    dst_width_ = dimensions[0];
    dst_height_ = dimensions[1];
  }

  return helper_.RunInGlContext([this, cc]() -> absl::Status {
    const auto& input = cc->Inputs().HasTag(kImageTag) ? cc->Inputs().Tag(kImageTag).Get<Image>().GetGpuBuffer() : TagOrIndex(cc->Inputs(), "VIDEO", 0).Get<GpuBuffer>();
    ...
    if (cc->Outputs().HasTag(kTopBottomPaddingTag) &&
      ...
      GetOutputPadding(src1.width(), src1.height(), dst_width, dst_height, &top_bottom_padding, &left_right_padding);
      cc->Outputs().Tag(kTopBottomPaddingTag).AddPacket(MakePacket<float>(top_bottom_padding).At(cc->InputTimestamp()));
      cc->Outputs().Tag(kLeftRightPaddingTag).AddPacket(MakePacket<float>(left_right_padding).At(cc->InputTimestamp()));
    }
...
void GlScalerCalculator::GetOutputDimensions(int src_width, int src_height, int* dst_width, int* dst_height) {
...
void GlScalerCalculator::GetOutputPadding(int src_width, int src_height, int dst_width, int dst_height, float* top_bottom_padding, float* left_right_padding) {
