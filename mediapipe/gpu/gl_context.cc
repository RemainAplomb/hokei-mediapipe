...
#define LINUX_STYLE_SETNAME_NP 1
  ...
  if (res != 0) {
    LOG_FIRST_N(INFO, 1) << "Can't set pthread names: name: \"" << name << "\"; error: " << res;
  }
#elif __APPLE__
...
absl::Status GlContext::GetGlExtensions() {
  ...
  if (!SymbolAvailable(&glGetStringi)) {
    LOG(ERROR) << "GL major version > 3.0 indicated, but glGetStringi not " << "defined. Falling back to deprecated GL extensions querying " << "method.";
...
absl::Status GlContext::Run(GlStatusFunction gl_func, int node_id, Timestamp input_timestamp) {
  ...
  if (profiling_helper_) {
    gl_func = [=] {
      profiling_helper_->MarkTimestamp(node_id, input_timestamp, /*is_finish=*/false);
      auto status = gl_func();
      profiling_helper_->MarkTimestamp(node_id, input_timestamp, /*is_finish=*/true);
      return status;
    };
  }
  if (thread_) {
...
absl::Status GlContext::SwitchContext(ContextBinding* saved_context, const ContextBinding& new_context)
