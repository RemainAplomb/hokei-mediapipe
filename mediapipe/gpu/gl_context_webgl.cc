...
GlContext::StatusOrGlContext GlContext::Create( EMSCRIPTEN_WEBGL_CONTEXT_HANDLE share_context, bool create_thread) {
...
absl::Status GlContext::CreateContextInternal(  EMSCRIPTEN_WEBGL_CONTEXT_HANDLE external_context, int webgl_version) {
  ...
  attrs.failIfMajorPerformanceCaveat = 1;
  ...
  EMSCRIPTEN_WEBGL_CONTEXT_HANDLE context_handle = emscripten_webgl_create_context(nullptr, &attrs);

  // Check for failure
  if (context_handle <= 0) {
    LOG(INFO) << "Couldn't create webGL " << webgl_version << " context.";
    return ::mediapipe::UnknownErrorBuilder(MEDIAPIPE_LOC) << "emscripten_webgl_create_context() returned error " << context_handle;
  } else {
...
absl::Status GlContext::CreateContext( EMSCRIPTEN_WEBGL_CONTEXT_HANDLE external_context) {
  ...
  // LOG(INFO) << "Successfully created a WebGL context with major version " << gl_major_version_ << " and handle " << context_;
...
void GlContext::DestroyContext() {
  ...
  if (context_ != 0) {
    ...
    if (res != EMSCRIPTEN_RESULT_SUCCESS) {
      ...
      LOG(INFO) << "Successfully destroyed WebGL context with handle " << context_;
    }
    context_ = 0;
  }
}
...
absl::Status GlContext::SetCurrentContextBinding(
  ...
  EMSCRIPTEN_RESULT res = emscripten_webgl_make_context_current(new_binding.context);
  RET_CHECK(res == EMSCRIPTEN_RESULT_SUCCESS) << "emscripten_webgl_make_context_current() returned error " << res;
  return absl::OkStatus();
}
