...
GpuResources::StatusOrGpuResources GpuResources::Create(PlatformGlContext external_context) {
  ASSIGN_OR_RETURN( std::shared_ptr<GlContext> context, GlContext::Create(external_context, kGlContextUseDedicatedThread));
...
GpuResources::GpuResources(std::shared_ptr<GlContext> gl_context) {
  ...
  ios_gpu_data_ = [[MPPGraphGPUData alloc] initWithContext:gl_context.get()multiPool:&gpu_buffer_pool_];
#endif  // __APPLE__
}
...
absl::Status GpuResources::PrepareGpuNode(CalculatorNode* node) {
  ...
  bool gets_own_context = (node_type == "ImageFrameToGpuBufferCalculator") ||
                          ...
                          (node_type == "RenderGPUBufferToCanvasCalculator");
  ...
  ASSIGN_OR_RETURN(std::shared_ptr<GlContext> context, GetOrCreateGlContext(context_key));
...
GlContext::StatusOrGlContext GpuResources::GetOrCreateGlContext(const std::string& key) {
  ...
  if (it == gl_key_context_.end()) {
    ASSIGN_OR_RETURN(std::shared_ptr<GlContext> new_context, GlContext::Create(*gl_key_context_[SharedContextKey()], kGlContextUseDedicatedThread));
