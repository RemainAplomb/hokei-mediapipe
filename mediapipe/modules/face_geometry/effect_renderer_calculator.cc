  ...
  namespace
  {
    ...
    private:
          ...
          const std::string &texture_path)
      {
        ...
        // }
        const int data = 1024 * 1024 * 3;
        auto temp_buffer = absl::make_unique<uint8_t[]>(data);
        uint8_t *buffer = temp_buffer.get();
        ImageFrame output_image_frame(mediapipe::ImageFormat::SRGB, 1024,
                                      1024,
                                      ImageFrame::kGlDefaultAlignmentBoundary);

        std::memcpy(output_image_frame.MutablePixelData(), buffer,
                    data);
