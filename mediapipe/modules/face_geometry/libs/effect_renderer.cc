...
namespace mediapipe::face_geometry
{
  namespace
  {
    ...
    public:
          ...
          const ImageFrame &image_frame)
      {
        ...
        return absl::WrapUnique(new Texture(handle, GL_TEXTURE_2D, image_frame.Width(), image_frame.Height(),
                                            /*is_owned*/ true));
      }
    ...
    public:
      ...
      absl::Status SetColorbuffer(const Texture &colorbuffer_texture)
      {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, colorbuffer_texture.target(), colorbuffer_texture.handle(), /*level*/ 0);
        glBindTexture(colorbuffer_texture.target(), 0);

        // If the existing depth buffer has different dimensions, delete it.
        if (renderbuffer_handle_ && (viewport_width_ != colorbuffer_texture.width() || viewport_height_ != colorbuffer_texture.height()))
        {
        ...
        if (!renderbuffer_handle_)
        {
          ...
          RET_CHECK(renderbuffer_handle_) << "Failed to initialize an OpenGL renderbuffer!";
          glBindRenderbuffer(GL_RENDERBUFFER, renderbuffer_handle_);
          glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, colorbuffer_texture.width(), colorbuffer_texture.height());
    ...
    public:
      ...
      static absl::StatusOr<std::unique_ptr<Renderer>> Create()
      {
        ...
        static const GLchar *kVertSrc = R"(#version 300 es  
      ...
      void main() {
        ...
        static const GLchar *kFragSrc = R"(#version 300 es
      ...
      vec4 thd = vec4(0.0/255.0, 255.0/255.0, 0.0/255.0, 145.0/255.0);

    float circleshape(vec2 position, float radius){
    return step(radius, length(position - vec2(0.5f)));
    }
    ...
    vec4 color = texture(image_tex, v_tex_coord);

    vec3 pink = vec3(.21, 0.0, 0.0);
    vec4 camera = texture(image_tex, v_tex_coord);


    vec4 originalColor = camera;
    vec4 screenColor = originalColor;


    float dx = 4.5 / canvasSize.x;
    float dy = 4.5 / canvasSize.y;

    vec4 nextColor0 = texture(image_tex, vec2(v_tex_coord.x - dx, v_tex_coord.y - dy)).xyzw;
    vec4 nextColor1 = texture(image_tex, vec2(v_tex_coord.x + dx, v_tex_coord.y - dy)).xyzw;
    vec4 nextColor2 = texture(image_tex, vec2(v_tex_coord.x - dx, v_tex_coord.y + dy)).xyzw;
    vec4 nextColor3 = texture(image_tex, vec2(v_tex_coord.x + dx, v_tex_coord.y + dy)).xyzw;
    ...
    vec4 filler = vec4(1.0f);

    float pct = abs(sin(0.5));

    float pctb = abs(sin(1.0));
    vec4 colortwo = color * camera;
    vec4 yuva = vec4(0.0);
    yuva.x = colortwo.r * 0.299 + colortwo.g * 0.587 + colortwo.b * 0.114;
    yuva.y = colortwo.r * -0.169 + colortwo.g * -0.331 + colortwo.b * 0.5 + 0.5;
    yuva.z = colortwo.r * 0.5 + colortwo.g * -0.419 + colortwo.b * -0.081 + 0.5;
    yuva.w = 1.0;

        vec4 combine2  = vec4(vec4(( (yuva[0] > thd[0]) && 
				(yuva[1] >= thd[0]) && (yuva[1] <= thd[1]) && 
				(yuva[2] >= thd[2]) && (yuva[2] <=thd[3])) ? color.rgba : screenColor.rgba));
        ...
        combine2.rgb = adjustExposure(combine2.rgb, 0.1);
        combine2.rgb = adjustSaturation(combine2.rgb, 0.1);
        ...
        GlhCreateProgram(kVertSrc, kFragSrc, NUM_ATTRIBUTES, (const GLchar **)&kAttrName[0], kAttrLocation, &program_handle);
        ...
        GLint texture_uniform = glGetUniformLocation(program_handle, "image_tex");
        GLint canvasSize = glGetUniformLocation(program_handle, "canvasSize");
        GLint smoothFactor = glGetUniformLocation(program_handle, "factor");
        ...
        return absl::WrapUnique(new Renderer(program_handle, projection_mat_uniform,
                                             model_mat_uniform, texture_uniform, canvasSize, smoothFactor));
      }
          ...
          RenderMode render_mode) const
      {
        ...
        GLfloat smoothval;
        // LOG(INFO) << "Data is "<< superfloat;
        smoothval = superfloat;
        // render_target.Bind();
        ...
        glUniformMatrix4fv(projection_mat_uniform_, 1, GL_TRUE,projection_mat.data());
        glUniformMatrix4fv(model_mat_uniform_, 1, GL_TRUE, model_mat.data());

        glUniform1f(smoothFactor_, smoothval);
        glUniform2f(canvasSize_, 480, 640);
        // Draw the mesh.
        glDrawElements(mesh_3d.primitive_type, mesh_3d.index_buffer.size(),GL_UNSIGNED_SHORT, mesh_3d.index_buffer.data());
    ...
    private:
      ...
      Renderer(GLuint program_handle, GLint projection_mat_uniform,
               GLint model_mat_uniform, GLint texture_uniform, GLint canvasSize, GLint smoothFactor)
    ...
    public:
          ...
          GLuint dst_texture_name)
      {
        ...
        for (int i = 0; i < num_faces; ++i)
        {
          ...
          ASSIGN_OR_RETURN(face_pose_transform_matrices[i], Convert4x4MatrixDataToArrayFormat(face_geometry.pose_transform_matrix()), _ << "Failed to extract the face pose transformation matrix!");
        ...
        // printf("%f", renderable_face_meshes[1].vertex_buffer[1]);
        ...
        for (int i = 0; i < num_faces; ++i)
        {
          const std::array<float, 16> &face_pose_transform_matrix = face_pose_transform_matrices[i];
      ...
      ImageFrame &&effect_texture)
  {
    ...
    ASSIGN_OR_RETURN(std::unique_ptr<RenderTarget> render_target, RenderTarget::Create(), _ << "Failed to create a render target!");
    ASSIGN_OR_RETURN(std::unique_ptr<Renderer> renderer, Renderer::Create(), _ << "Failed to create a renderer!");
    ASSIGN_OR_RETURN(RenderableMesh3d renderable_quad_mesh_3d, RenderableMesh3d::CreateFromProtoMesh3d(CreateQuadMesh3d()), _ << "Failed to create a renderable quad mesh!");
    absl::optional<RenderableMesh3d> renderable_effect_mesh_3d;
    if (effect_mesh_3d)
    {
