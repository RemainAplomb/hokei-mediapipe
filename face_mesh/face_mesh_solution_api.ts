...
class GraphDataImpl {
  ...
  async toArrayBuffer(): Promise<ArrayBuffer> {
    console.log("Locate Graph:", this.locateFile(this.graph.url, this.packagePath))
    console.log("Locate Graph with dot:", `.${this.locateFile(this.graph.url, this.packagePath)}`);
    if (this.graph.url) {
...
export class Solution {
  ...
  private async loadFiles(files: string[]): Promise<void> {
    await Promise.all(files.map(async (file) => {
      try {
        const url = this.locateFile(file, this.packagePath);
  ...
  async loadGraph(graph: GraphData): Promise<void> {
    console.log("Initial Graph Data: ", graph)
