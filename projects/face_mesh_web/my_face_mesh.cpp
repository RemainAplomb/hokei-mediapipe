...
typedef std::vector<float> FaceGeometryMatrix;

typedef std::vector<FaceLandmark> FaceLandmarkList;

struct FaceGeometryContainer
{
	FaceGeometryMatrix face_geo_metrix;
};
...
public:
	...
	FaceGeometryContainer face_geometry_container;
	...
	bool isFaceGeometry()
	{
		if (!face_geometry_container.face_geo_metrix.empty())
		{
			return true;
		}
		else
	...
	FaceGeometryContainer getFaceGeometry()
	{
		return face_geometry_container;
	}
...
public:
	...
	void processFrame(std::string inputStringName, FrameMetaDataWasm frameInput)
	{
		...
		// console.log("frameInput: ", frameInput)
		// LOG(ERROR) << "inputStringName: " << status.message();
		LOG(ERROR) << "Failed to run the inputStringName: " << inputStringName;
		// LOG(ERROR) << "FrameMetaDataWasm: " << frameInput;
